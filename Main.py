...
class SatIntelTerminal:
	...
	def on_coord_select(self,event):
		if not self.loc_box.curselection():return
		...
		for i in self.tree.get_children():
			self.tree.delete(i)
		self.tree.insert("","end",values=("VGG16+AUTOENCODER CLASSIFIER",p['vgg_label'],p['vgg_conf']))
		self.tree.insert("","end",values=("---","GLCM FEATURE BREAKDOWN","---"))
		for f in p['features']:
